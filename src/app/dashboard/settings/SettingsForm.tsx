...
export default function SettingsForm({ org, initialRecipients, orgId }: Props) {
  ...
  async function saveOrgSettings() {
    ...
    await supabase
      .from('organizations')
      .update({ name: orgName, reply_to_email: replyTo, summary_send_time: summaryTime })
      .eq('id', orgId)
    setSavingOrg(false)
    setOrgSaved(true)
  ...
  async function addRecipient() {
    ...
    const { data } = await supabase
      ...
      .insert({ ...newRecipient, organization_id: orgId, receives_summary: true, receives_instant: true })
      .select()
      .single()
  ...
  async function toggleRecipient(id: string, field: 'receives_summary' | 'receives_instant') {
    ...
    await supabase.from('notification_recipients').update({ [field]: newVal }).eq('id', id)
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            />
            <p className="text-xs text-slate-400 mt-1">Cron is set to 8:00 AM EST by default (edit vercel.json to change).</p>
          </div>
        </div>
        <button
      ...
      <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-6">
        ...
        <p className="text-sm text-slate-600 mb-3">
          Share this link with your staff so they can report absences.
        </p>
